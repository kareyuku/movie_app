...
export class ReviewsService {
  ...
  async createReview(
    ...
    if(await this.reviewModel.findOne({addedBy: user, addedTo: anime._id}))
      throw new BadRequestException("Recenzja do tego filmu już istnieje.")

    const newRate = parseFloat( ((anime.rate + reviewDto.rate) / 2).toFixed(1) );
